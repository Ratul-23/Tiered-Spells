...
def build_spells(
    object_name_by_group: dict[int, str], spell_data_by_path: dict[str, SpellData]
) -> list[Spell]:
    """
    ...
    for tiered_group, object_name in object_name_by_group.items():
        ...
        spells.append(
            Spell(spell_data.school, tiered_group, spell_data.name_locale_code)
        )
...
def resolve_display_name(
    ...
    for spell in spells:
        ...
        if locale_code in exceptions:
            spell.display_name: str = exceptions[locale_code]

        else:
            spell.display_name: str = string_by_locale_code.get(locale_code, "")
...
def build_tiered_spells(
    ...
    spell_by_key: dict[tuple[str, str], Spell] = {
        (spell.name_locale_code, spell.school): spell for spell in spells
    }

    for path, spell_data in spell_data_by_path.items():
        spell: Optional[Spell] = spell_by_key.get(
            (spell_data.name_locale_code, spell_data.school)
        )
        ...
        tiered_spell: TieredSpell = TieredSpell(
            spell_data.object_name, path, spell_data.desc_locale_code, spell_id
        )
...
def resolve_tiered_spells(
    spells: list[Spell], string_by_locale_code: dict[str, str]
) -> list[Spell]:
    """
    ...
    for spell in spells:
        for tiered_spell in spell.tiered_spells:
            tiered_spell.desc: str = string_by_locale_code.get(
                tiered_spell.desc_locale_code, ""
            )

            match: Optional[re.Match[str]] = TIER_PATH_PATTERN.search(
                tiered_spell.object_name
            )
...
def main() -> None:
    ...
    spells: list[Spell] = resolve_display_name(
        spells, string_by_locale_code, LOCALE_CODE_EXCEPTIONS
    )

    spells: list[Spell] = build_tiered_spells(
        spells, spell_data_by_path, spell_id_by_path
    )

    spells: list[Spell] = resolve_tiered_spells(spells, string_by_locale_code)
